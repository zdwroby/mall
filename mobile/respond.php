...
$order = $db->getRow("SELECT * FROM " . $ecs->table('order_info') . " WHERE `user_id` = '$user_id' ORDER BY order_id DESC LIMIT 1");
if($order &&  $order['extension_code']=='pintuan'){
	$order_id=$order['order_id'];
	$sql = "SELECT * FROM " . $ecs->table('pintuan_orders') . " WHERE order_id = '$order_id' ";
	$pintuan = $db->getRow($sql);
	if($pintuan){
	$pt_id=$pintuan['pt_id'];
	$follow_user=$pintuan['follow_user'];
	$url = 'pintuan.php?act=pt_view&pt_id='.$pt_id.'&u='.$follow_user;
