 ...
 *  修改奖金比例
 */
elseif ($_REQUEST['act'] == 'edit_scale')
{
    check_authz_json('user_rank');
    ...
    if ($exc->edit("award_scale = '$val'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
         admin_log(addslashes($rank_name), 'edit', 'user_rank');
         clear_cache_files();
...
//-- 切换是否开启提成
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'award_on')
{
    check_authz_json('user_rank');

    $rank_id       = intval($_POST['id']);
    $award_on    = intval($_POST['val']);

    if ($exc->edit("award_on = '$award_on'", $rank_id))
    {
        $rank_name = $exc->get_name($rank_id);
        admin_log(addslashes($rank_name), 'edit', 'user_rank');
        clear_cache_files();
        make_json_result($award_on);
    }
    else
    {
        make_json_error($db->error());
