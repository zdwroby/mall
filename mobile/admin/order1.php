...
/*------------------------------------------------------ */
    ...
    elseif (isset($_POST['remove']))
    {
        ...
        if (!$batch)
        {
            ...
            $db->query("DELETE FROM ".$ecs->table('pintuan_orders'). " WHERE order_id = '$order_id'");//拼团
...
elseif ($_REQUEST['act'] == 'batch_operate_post')
{
    ...
    elseif ('remove' == $operation)
    {
        foreach ($order_id_list as $id_order)
        {
            ...
            $db->query("DELETE FROM ".$ecs->table('pintuan_orders'). " WHERE order_id = '$order[order_id]'"); //拼团
            $action_array = array('delivery', 'back');
...
elseif ($_REQUEST['act'] == 'remove_order')
{
    ...
    $GLOBALS['db']->query("DELETE FROM ".$GLOBALS['ecs']->table('pintuan_orders'). " WHERE order_id = '$order_id'");// 拼团
    $action_array = array('delivery', 'back');
    del_delivery($order_id, $action_array);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['pintuan'] = isset($_REQUEST['pintuan']) ? intval($_REQUEST['pintuan']) : 0;
        ...
        if ($filter['pintuan'])
        {
            $where .= " AND o.extension_code = 'pintuan'  ";
        }
