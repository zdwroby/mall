...
elseif($_REQUEST['act'] == 'ewm')
{
	admin_priv('wx_api');
	
	$wxch_lang['ur_here'] = '二维码设置';
	if(!empty($_POST['hidden_type']))
	{
		$hearimg_x = $_POST['hearimg_x'];
		$hearimg_y = $_POST['hearimg_y'];
		$hearimg_width = $_POST['hearimg_width'];
		$hearimg_hight = $_POST['hearimg_hight'];
		$qr_x = $_POST['qr_x'];
		$qr_y = $_POST['qr_y'];
		$qr_width = $_POST['qr_width'];
		$qr_hight = $_POST['qr_hight'];
		$text_red = $_POST['text_red'];
		$text_geren = $_POST['text_geren'];
		$text_blue = $_POST['text_blue'];
		if(!empty($_FILES['bg_img']['tmp_name']))
		{
			if ((isset($_FILES['bg_img']['error']) && $_FILES['bg_img']['error'] == 0) || (!isset($_FILES['bg_img']['error']) && isset($_FILES['bg_img']['tmp_name']) && $_FILES['bg_img']['tmp_name'] != 'none'))
			{
				$img_up_info = wxch_upload_file_erweima($_FILES['bg_img'],'qrcode');
			}
			if (((isset($_FILES['bg_img']['error']) && $_FILES['bg_img']['error'] > 0) || (!isset($_FILES['bg_img']['error']) && isset($_FILES['bg_img']['tmp_name']) && $_FILES['bg_img']['tmp_name'] == 'none')) )
			{
		...
		{
			$img_up_info = $db->getOne("select bg_img from wxch_erweima where id=1");
		}	
		$ret = $db->query("UPDATE `wxch_erweima` SET 
				`hearimg_x`='$hearimg_x',
				`hearimg_y`='$hearimg_y',
				`hearimg_width`='$hearimg_width',
				`hearimg_hight`='$hearimg_hight',
				`qr_x`='$qr_x',
				`qr_y`='$qr_y',
				`qr_width`='$qr_width',
				`qr_hight`='$qr_hight',
				`bg_img` = '$img_up_info',
				`text_red` = '$text_red',
				`text_geren` = '$text_geren',
				`text_blue` = '$text_blue'		
				WHERE `id`=1;");
		$link[] = array('href' =>'wxch-ent.php?act=ewm', 'text' => '二维码设置');
		if($ret)
		{
			sys_msg('甜心100提示您设置成功',0,$link);
		}
	...
	{
		$ret = $db->getRow("SELECT * FROM `wxch_erweima` WHERE `id` = 1");
		$smarty->assign('ret',$ret);
		$smarty->assign('wxch_lang',$wxch_lang);
		$smarty->display('wxch_wxewm.html');
	}
}
elseif($_REQUEST['act'] == 'look_erweima')
{
	$wxch_lang['ur_here'] = '二维码预览';
	...
	//$bg_imgsrc = 'http://'.$this_url.'/xiaojunectouch/mobile/'.$bg_img;
	$bg_imgsrc = 'http://'.$this_url.'/mobile/'.$bg_img;
	
	//$qr_imgsrc = 'http://'.$this_url.'/xiaojunectouch/qrcode/1450788973.jpg';
	$qr_imgsrc = 'http://'.$this_url.'/mobile/qrcode/demo.jpg';
	if(!getimagesize($qr_imgsrc))
	{
		print_r("请在项目文件夹下的mobile/qrcode下上传一张名为demo.jpg 的二维码图片，以便于预览");
		exit;
	}
	//$h_imgsrc='http://'.$this_url.'/xiaojunectouch/qrcode/1450788973_1.jpg';
	$h_imgsrc='http://'.$this_url.'/mobile/qrcode/demo_1.jpg';
	if(!getimagesize($h_imgsrc))
	{
		print_r("请在项目文件夹下的mobile/qrcode下上传一张名为demo_1.jpg 的头像图片，以便于预览");
		exit;
	}
	$time = time();
    //处理二维码
	$name =resizejpg($qr_imgsrc,$qr_width,$qr_height,$time);
	$imgs = $name;
	//处理头像
	$h_time=$time."-1";
	$h_name=resizejpg($h_imgsrc,$hearimg_width,$hearimg_hight,$h_time);
	$h_imgs = $h_name;	
	
	if(!empty($bg_img))
	{
		$target = $bg_imgsrc;//背景图片 mobile下的qrcode 文件下
	}
	else
	{
		//$target = 'http://'.$this_url.'/xiaojunectouch/wechat/qrcode/tianxin100.jpg';//背景图片
		$target = 'http://'.$this_url.'/wechat/qrcode/tianxin100.jpg';//背景图片
	}
	...
	//$textcolor = imagecolorallocate($target_img, 0, 255, 0);
	...
	/* $bg_image_info = getimagesize($bg_imgsrc);
	 $type = image_type_to_extension($bg_image_info[2],false);
	 $falge = 'imagecreatefrom'.$type;
	 $image = $falge($bg_imgsrc);
	
	 //操作图片
	 $qr_imgsrc = 'http://'.$this_url.'/xiaojunectouch/qrcode/1450186593.jpg';
	 $info2 = getimagesize($qr_imgsrc);
	 $type2 = image_type_to_extension($info2[2],false);
	 $falge2 = 'imagecreatefrom'.$type2;
	 $water = $falge2($qr_imgsrc);
	 ...
	 header("Content-type:".$bg_image_info['mime']);
	 imagejpeg($image,'../qrcode/234.'.$type);
	 //$funcs ="image".$type;
	 //$funcs($image);
	 //$funcs($image,'http://localhost/xiaojunectouch/qrcode/sdf.'.$type);
	 imagedestroy($image); */
	...
	$smarty->assign('wxch_lang',$wxch_lang);
	$smarty->display('wxch_lookerweima.html');
}
...
*/
	$this_url =  $_SERVER['HTTP_HOST'];
	//$qr_imgsrc = $this_url.'/xiaojunectouch/qrcode/1450186593.jpg';
	$name="../qrcode/".$time.".jpg";
...
function wxch_upload_file_erweima($upload,$dir)
{
	$image = new cls_image();
