...
if ($_REQUEST['act'] == 'list') 
{
	...
	$record_count = $db->getOne("SELECT count( uid ) FROM `wxch_user` where subscribe=1 ");
	$filter['page'] = 1;
	$filter['page_size'] = 15;
	$full_page = 1;
	if($filter['page'] <=1)
	...
	$sql = "SELECT * FROM `wxch_user` where subscribe=1 ORDER BY `dateline`  DESC LIMIT $start , $filter[page_size]";
	$ret = $db->getAll($sql);
	foreach($ret as $k=>$v)
	{
...
elseif($_REQUEST['act'] == 'query') 
{
	...
	if(!empty($uid)) 
	{
		$sql = "SELECT * FROM `wxch_user` WHERE `uid` = '$uid' and subscribe=1";
		$w_user = $db->getRow($sql);
	}
	...
	elseif(!empty($_POST['keywords'])) 
	{
		...
		$filter['record_count'] = $db->getOne("SELECT count( wxid ) FROM `wxch_user` WHERE `$row_type` LIKE '%$keyword%' and subscribe=1");
		if (isset($_REQUEST['page_size']) && intval($_REQUEST['page_size']) > 0) 
		{
			$filter['page_size'] = intval($_REQUEST['page_size']);
		}
		...
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'view';
		$sql = "SELECT * FROM `wxch_user` WHERE `$row_type` LIKE '%$keyword%' and subscribe=1 ORDER BY `uid` DESC LIMIT $filter[start] , $filter[page_size]";
		$ret = $db->getAll($sql);
		foreach($ret as $k=>$v)
		{
			$v['subscribe_time'] = date("Y-m-d",$v['subscribe_time']);
	...
	elseif($type == 'list') 
	{
		...
		$filter['start'] = ($filter['page'] - 1) * $filter['page_size'];
		$filter['type'] = 'list';
		$sql = "SELECT * FROM `wxch_user` where subscribe=1 ORDER BY `uid` DESC LIMIT $filter[start] , $filter[page_size]";
		$ret = $db->getAll($sql);
		foreach($ret as $k=>$v)
		{
			$v['subscribe_time'] = date("Y-m-d",$v['subscribe_time']);
