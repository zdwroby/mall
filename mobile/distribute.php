...
elseif($action == 'userrank'){
	...
	{
		$sql = "SELECT  sum(goods_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=".$value[user_id]." and order_status = '5' and shipping_status = '2'";
	...
	{
		$sql = "SELECT  sum(goods_amount ) as sum_order  FROM " . $GLOBALS['ecs']->table('order_info') . " WHERE user_id=$value and order_status = '5' and shipping_status = '2'";
		$count = $GLOBALS['db']->getOne($sql);
...
elseif ($action == 'fenxiao')
{
    ...
    foreach($user_info as $key=>$value){
        ...
        $sql="SELECT count(*) as order_num ,sum(sales".$rank."_money)   as order_amount ,sum(order_amount) as all_sales  FROM ".$GLOBALS['ecs']->table('order_info')."WHERE user_id=".$value['user_id']." and order_status not in (2,4,7) " ;
        $order_info=$db->getRow($sql);
    ...
    if(!empty($affiliate['on']))
    {
        if(empty($separate_by))
        {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    ...
                    " WHERE o.user_id > 0 AND (u.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd and o.order_status not in (2,4,7)";
        }
        else
        {
            //推荐订单分成
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                    " LEFT JOIN".$GLOBALS['ecs']->table('users')." u ON o.user_id = u.user_id".
                    " LEFT JOIN " . $GLOBALS['ecs']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                    " WHERE o.user_id > 0 AND (o.parent_id > 0 AND o.is_separate = 0 OR o.is_separate > 0) $sqladd and o.order_status not in (2,4,7)";
        }
    }
    else
    {
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('order_info') . " o".
                " LEFT JOIN".$GLOBALS['ecs']->table('users')." u ON o.user_id = u.user_id".
                " LEFT JOIN " . $GLOBALS['ecs']->table('affiliate_log') . " a ON o.order_id = a.order_id" .
                " WHERE o.user_id > 0 AND o.is_separate > 0 $sqladd and o.order_status not in (2,4,7)";
    }
